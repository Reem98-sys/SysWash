...
class _PickupdetailsState extends State<Pickupdetails> {
  ...
  void _recalculateTotals() {
    ...
    for (var item in _orderItemsList) {
      final price = double.tryParse(item['clothPrice']?.toString() ?? '0') ?? 0;
      final qnty = int.tryParse(item['qnty']?.toString() ?? '0') ?? 0;
    ...
    showDialog(
      ...
      builder: (_) {
        return AlertDialog(
          ...
          title: const Row(children: [Text("Reschedule Pickup")]),
          content: const Text(
            "Are you sure you want to reschedule this pickup order?",
          ),
          actions: [
            TextButton(
              onPressed: () {
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (token != null && companyCode != null) {
                  context.read<ResheduleorderBloc>().add(
                    FetchResheduleOrderEvent(
                      id: widget.pickupAssignId.toString(),
                      token: token,
                      companyCode: companyCode,
                    ),
                  );
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: MultiBlocListener(
            listeners: [
              BlocListener<ResheduleorderBloc, ResheduleorderState>(
              listener: (context, state) {
                if (state is ResheduleOrderLoaded) {
                ...
                if (state is ResheduleOrderError) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Unable to Reshedule')),
                  );
                }
              },
            ),
              ...
              builder: (context, state) {
                ...
                if (state is PickupCustDetailsLoaded) {
                  ...
                  final gridItems = [
                    ...
                    {
                      'label': 'Street No',
                      'value': customerDetailsModel.streetNo,
                    },
                    {
                      'label': 'Villa No',
                      'value': customerDetailsModel.villaNumber,
                    },
                    {'label': 'Hotel', 'value': customerDetailsModel.hotel},
                    {'label': 'Room No', 'value': customerDetailsModel.roomNo},
                    {
                      'label': 'Fragrance',
                      'value': customerDetailsModel.fragrance,
                    },
                    {
                      'label': 'Reference No',
                      'value': customerDetailsModel.refNo,
                    },
                    {'label': 'Note', 'value': widget.notes},
                    {'label': 'Remark', 'value': widget.remarks},
                    {
                      'label': 'Order No',
                      'value': widget.pickupOrderId?.toString() ?? '',
                    },
                    {
                      'label': 'Pickup Id',
                      'value': widget.pickupAssignId.toString(),
                    },
                  ];
                  return Column(
                    children: [
                                    ...
                                    onTap: () async {
                                      ...
                                      if (coords == null) {
                                        final fullAddress = buildFullAddress(
                                          ...
                                          area: customerDetailsModel.area,
                                          zone: customerDetailsModel.zone,
                                          street: customerDetailsModel.streetNo,
                                          villano:
                                              customerDetailsModel.villaNumber,
                                        );
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  //case 2 Adding items to existing ordered items
                                    ...
                                    if (pickupOrderItems.customerDiscount !=
                                        0) {
                                      totalDiscount =
                                          subtotal *
                                    ...
                                    if (_orderItemsList.isNotEmpty &&
                                      ...
                                      if (_orderItemsList.length != 0) {
                                        context
                                            .read<PickupcustdetailsBloc>()
                                            .add(
                                              FetchStatusPickupEvent(
                                                pickupAssignId:
                                                    widget.pickupAssignId,
                                                token: token,
                                                companyCode: companyCode,
                                                status: 'Received',
                                              ),
