...
class SysRepository {
  ...
  var body = {};
  Future<LoginModel> loginIn(String email,String password,String companyCode) async {
  ...
  Future<TotalOrderModel> totalOrder(String userId,String companyCode,String token) async {
  String url = "https://be.syswash.net/api/syswash/historylist/${userId}?code=${companyCode}";
  var body = {};
  Response response = await apiClient.invokeAPI(url, "GET", jsonEncode(body),token: token);
  return TotalOrderModel.fromJson(jsonDecode(response.body));
  }
  Future<PickUpListResponse> pickUpList(
    String userId, String companyCode, String token) async {
  String url =
      "https://be.syswash.net/api/syswash/pickuplist/$userId?code=$companyCode";

  Response response =
      await apiClient.invokeAPI(url, "GET", jsonEncode({}), token: token);

  final decoded = jsonDecode(response.body);
  print('🚀 pickupList API response type: ${decoded.runtimeType}');
